...
function Profile({ id }) {
  ...
  useEffect(() => {
    async function fetchProfile() {
      ...
      if (profileData) {
        ...
        const countryDetail = countries.filter(item => item?.name.toLowerCase() === profileData?.country?.toLowerCase())?.[0]
  ...
  async function handleSubmit(event) {
    ...
    const profileData = {
      ...
      current_location: auth?.user?.githubDetails?.location,
          ...
          <div className="mt-6 grid grid-cols-12 gap-6">
            ...
            <div className="col-span-12 sm:col-span-6">
              ...
              <input
                ...
                autoComplete="email"
                className=" bg-[#0b0b0c] block w-full rounded-md border-0 px-3 py-1.5 shadow-sm placeholder:text-gray-400 focus:border-0 sm:text-sm sm:leading-6"
              />
            </div>
