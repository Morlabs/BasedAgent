...
export async function addDeveloper(user) {
	try {
		...
		console.log("Existing user Name Details:", existingUser?.name , existingUser?.githubUsername);
		...
		const dataChanged = (existingUser.name !== user.name || existingUser.email !== user.email || existingUser.skills.join(', ') !== skillsArray.join(', ') ||  // Compare skills as strings
			existingUser.publicRepositories !== user.githubDetails.public_repos || existingUser.imageUrl !== user.image || existingUser?.location !== user?.githubDetails?.location);
		...
		if (dataChanged) {
			...
			await db.update(developers)
				.set({
					...
					publicRepositories: user.githubDetails.public_repos, imageUrl: user.image,
					location: user?.githubDetails?.location
